...
export const TournamentRegistrationWizard: React.FC<WizardProps> = ({
  ...
  // Check if any selected event requires a partner (doubles)
  ...
  useEffect(() => {
    ...
    // Load existing teams for this player (to detect prior registrations)
    ...
    loadExistingTeams();
    // Load or create the registration record
    const initReg = async () => {
      ...
      setRegData(reg);
      // If any partner selections already exist in the registration, load them
  ...
  const handleSave = async (updates: Partial<TournamentRegistration>) => {
    if (!regData) return;
    const updatedReg = { ...regData, ...updates };
    setRegData(updatedReg);
    await saveRegistration(updatedReg as TournamentRegistration);
  };

  const handleFinalize = async () => {
    setError(null);
    if (!regData) return;
    // 1) Validate partner choices for all selected doubles events (skip in waiver-only mode)
    if (!isWaiverOnly) {
      ...
      for (const divId of regData.selectedEventIds) {
        ...
        switch (details.mode) {
          ...
          case 'open_team':
          default:
            // "I don't have a partner yet" is valid without extra info
            break;
        }
      }
      if (missing.length > 0) {
  ...
  // Dynamic label for the final action button
      ...
      <div className="bg-gray-800 w-full max-w-2xl p-6 rounded-lg border border-gray-700 relative flex flex-col max-h-[90vh]">
        ...
        <div className="overflow-y-auto flex-grow pr-2">
          <>
            ...
            {step === 1 && (
              <div className="space-y-4">
                <p className="text-gray-300">
                  {initialDivisionId ? 'You have been invited to join the following event:' : 'Select Division(s):'}
                </p>
                {isRegistrationComplete && !isWaiverOnly && (
                  <p className="text-xs text-gray-400">
                    You are already registered. You can update your divisions and partner options below, then click{' '}
                    <span className="font-semibold text-gray-200">Update Registration</span> to save your changes.
                  </p>
                )}
                <div className="grid gap-3">
                  {divisions.map(div => {
                    ...
                    // (Removed filtering: show all divisions even in invite mode)
                    return (
                      <div
                        key={div.id}
                        onClick={() => {
                          if (!eligible) return;
                          if (isWaiverOnly && div.id === initialDivisionId) return; // in invite flow, lock the invited event selection
                        ...
                        className={`p-4 rounded border flex justify-between items-center transition-all ${
                          !eligible
                            ...
                            : isSelected
                              ? `bg-green-900/40 border-green-500 ${isWaiverOnly && div.id === initialDivisionId ? 'cursor-not-allowed' : 'cursor-pointer'} shadow-[0_0_10px_rgba(34,197,94,0.1)]`
                              : 'bg-gray-700 border-gray-600 hover:bg-gray-600 cursor-pointer'
                        }`}
                      >
                        <div>
                          <div className={`font-bold ${isSelected ? 'text-green-400' : 'text-white'}`}>
                            {div.name}
                          </div>
                ...
                <div className="flex justify-between items-center mt-6 border-t border-gray-700 pt-4">
                  ...
                  <button
                    onClick={() => {
                      // If no doubles selected, or only the invited doubles event selected, skip partner step
                      const invitedDiv = initialDivisionId ? divisions.find(d => d.id === initialDivisionId) : undefined;
                      const invitedIsDouble = invitedDiv?.type === 'doubles';
                      const selectedDoubles = regData.selectedEventIds.filter(id => {
                        const d = divisions.find(div => div.id === id);
                        return d?.type === 'doubles';
                      });
                      if (!hasDoublesSelection || (invitedIsDouble && selectedDoubles.length === 1)) {
                        setStep(3);
                      } else {
                        setStep(2);
                      }
            ...
            {/* STEP 2 – Partner Selection (skipped if no additional doubles selected) */}
            {step === 2 && !isWaiverOnly && hasDoublesSelection && (
              <div className="space-y-6">
                <p className="text-gray-300 font-medium">Partner Selection (For doubles events):</p>
                <DoublesPartnerStep
                  tournament={tournament}
                  divisions={divisions}
                  selectedDivisionIds={
                    // Exclude the invited division (if any) from partner selection, since it already has a partner
                    regData.selectedEventIds.filter(id => {
                      if (!initialDivisionId) return true;
                      const div = divisions.find(d => d.id === id);
                      if (!div || div.type !== 'doubles') return false;
                      return !(id === initialDivisionId);
                    })
                  }
                  userProfile={userProfile}
            ...
            {/* STEP 3 – Waiver + Summary + Finalize */}
            {step === 3 && (
              <div className="space-y-4">
                        ...
                        <li key={divId}>
                          ...
                          {!isWaiverOnly && div?.type === 'doubles' && details && (
                            <span className="text-gray-400">
                              {details.mode === 'invite' && details.partnerName ? ` with ${details.partnerName}` : ''}
                              {details.mode === 'open_team' && ' (Looking for partner)'}
                              {details.mode === 'join_open' && ' (Joining open team)'}
                            </span>
                          )}
                          {isWaiverOnly && (
                            <span className="text-gray-400"> (Accepted Invite)</span>
                          )}
                        </li>
                      );
                    })}
