...
export const DevTools: React.FC<{ onBack: () => void }> = ({ onBack }) => {
    ...
    const runLeagueIntegrationTest = async () => {
        ...
        try {
            ...
            const matchesSnap = await db.collection('matches').where('competitionId', '==', testCompId).get();
            ...
            const standingSnapA = await db.collection('standings').doc(standingIdA).get();
            ...
            batch.delete(db.collection('competitions').doc(testCompId));
            batch.delete(db.collection('competitionEntries').doc(entry1Id));
            batch.delete(db.collection('competitionEntries').doc(entry2Id));
            matchesSnap.forEach(m => batch.delete(m.ref));
            batch.delete(db.collection('standings').doc(standingIdA));
            batch.delete(db.collection('standings').doc(`${testCompId}_${teamB}`));
            const subsSnap = await db.collection('matchScoreSubmissions').where('matchId', '==', matchDoc.id).get();
    ...
    const runTeamLeagueIntegrationTest = async () => {
        ...
        try {
            ...
            await new Promise(r => setTimeout(r, 2000));
            const sSnap = await db.collection('standings').doc(`${testCompId}_${divId}_${teamAId}`).get();
            ...
            const batch = db.batch();
            batch.delete(db.collection('competitions').doc(testCompId));
            batch.delete(db.collection('teams').doc(teamAId));
            batch.delete(db.collection('teams').doc(teamBId));
