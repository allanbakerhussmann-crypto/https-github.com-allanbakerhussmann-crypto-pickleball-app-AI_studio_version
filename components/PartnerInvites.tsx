...
import type { PartnerInvite, Tournament, Division, UserProfile, TournamentRegistration } from '../types';
...
export const PartnerInvites: React.FC<PartnerInvitesProps> = ({ onAcceptInvite }) => {
  ...
  useEffect(() => {
    ...
    // 1. Subscribe to partner invites for the current user
  ...
  // 2. Fetch tournaments and user profiles related to invites when invite list changes
  useEffect(() => {
    if (invites.length === 0) return;
    const loadMetadata = async () => {
      const tIds = Array.from(new Set(invites.map(i => i.tournamentId)));
      const uIds = Array.from(new Set(invites.map(i => i.inviterId)));
      // Fetch all tournaments (could be optimized to specific IDs if available)
      const allTournaments = await getAllTournaments(100);
      const tMap: Record<string, Tournament> = {};
      allTournaments.forEach(t => { tMap[t.id] = t; });
      // Fetch inviter profiles in batch
  ...
  const handleRespond = async (invite: PartnerInvite, response: 'accepted' | 'declined') => {
    try {
      ...
      if (response === 'accepted' && result && currentUser && onAcceptInvite) {
        // Ensure a registration exists (or is updated) for this tournament and player
        await ensureRegistrationForUser(result.tournamentId, currentUser.uid, result.divisionId);
        // If invite accepted, pre-set partner details for the invited division
        if (invite.inviterId) {
          const inviterProfile = inviters[invite.inviterId];
          const partnerName = inviterProfile ? (inviterProfile.displayName || inviterProfile.email || '') : '';
          const reg = await getRegistration(result.tournamentId, currentUser.uid);
          if (reg) {
            reg.partnerDetails = {
              ...(reg.partnerDetails || {}),
              [result.divisionId]: {
        ...
        // Navigate to the full registration wizard for this tournament
  ...
  if (loading) {
    return <div className="p-8 text-center text-gray-500">Loading invites...</div>;
  }
      ...
      {invites.length === 0 ? (
        <div className="bg-gray-800 p-8 rounded-lg border border-gray-700 text-center">
          ...
          <p className="text-gray-400 mt-2">
            When someone invites you to be their partner, it will show up here.
          </p>
        </div>
      ) : (
        <div className="grid gap-4">
          {invites.map(invite => {
            ...
            return (
              <div 
                key={invite.id} 
                className="bg-gray-800 p-6 rounded-lg border border-gray-700 shadow-lg flex flex-col md:flex-row justify-between items-center gap-4"
              >
                <div>
                  <div className="text-xs font-bold text-green-400 uppercase tracking-wider mb-1">Partner Request</div>
